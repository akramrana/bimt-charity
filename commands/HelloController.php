...
class HelloController extends Controller {
    ...
    public function actionSendInvoiceEmails() {
        $models = \app\models\MonthlyInvoice::find()
                ->where([
                    'instalment_month' => date('F'),
                    'instalment_year' => date('Y'),
                    'is_deleted' => 0,
                    'generation_email_sent' => 0,
                ])
                ->orderBy(['monthly_invoice_id' => SORT_ASC])
                ->limit(90)
                ->all();
        ...
        foreach ($models as $model) {
            ...
            $result = \app\helpers\AppHelper::resendEmail($mailObject);
            if ($result) {
                $model->generation_email_sent = 1;
                $model->save(false, ['generation_email_sent']);
            }
        }
